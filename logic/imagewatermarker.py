...
class ImageWatermarker:
    ...
    def _add_current_date(self, image, size_ratio):
        """
        Adds current date to the bottom-left of the image.
        """
        draw = ImageDraw.Draw(image)
        date_str = datetime.now().strftime("%Y-%m-%d")
        img_width, img_height = image.size
        
        # Use a fraction of the watermark size ratio for the date font size
        font_size = int(img_height * size_ratio * 0.5) 
        ...
        pos = (padding, img_height - th - padding)
        draw.text((pos[0]+1, pos[1]+1), date_str, font=font, fill=shadow_color)
        draw.text(pos, date_str, font=font, fill=text_color)
        return image

    def apply_watermark_image(self, input_path, output_path, size_ratio, opacity_ratio, add_date=False):
        """
        ...
        try:
            ...
            if add_date:
                base_image = self._add_current_date(base_image, size_ratio)
    ...
    def apply_watermark_text(self, input_path, output_path, text_details, size_ratio, opacity_ratio, add_date=False):
        """
        ...
        try:
            ...
            if add_date:
                watermarked_image = self._add_current_date(watermarked_image, size_ratio)
